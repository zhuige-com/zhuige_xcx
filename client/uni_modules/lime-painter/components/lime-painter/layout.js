...
export class Layout {
	...
	getComputedStyle(element, parent = {}, index = 0) {
		...
		for (let value of Object.keys(node)) {
			...
			if(/%/.test(item)) {
				...
				style[value] = /^[\d\.]+(px|rpx)?$/.test(item)? toPx(item) : /em$/.test(item) && name == 'text'? toPx(item, node['fontSize']) : item
			}
		}
	...
	async getAttributes(element) {
		...
		if(element?.url || element?.src) {
			...
			const {width = 0, height = 0, path: src, originSrc} = await getImageInfo(arr.src, this.isH5PathToBase64) || {}
			arr = Object.assign({}, arr, {width, height, src, originSrc})
		}
		if(element?.text) {
	...
	async calcNode(element) {
		const node = await this.getNodeTree(element)
